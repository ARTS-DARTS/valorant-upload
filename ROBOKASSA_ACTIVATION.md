...
3. Убедиться, что Robokassa вернула на `/payment/success?InvId=...`, а кнопка
   «Вернуться в приложение» открывает
   `vlineupapp://billing/success?orderId=...`.
...
11. Проверить отмену оплаты: `/payment/fail` не выдаёт права, а возврат по
    `vlineupapp://billing/fail?orderId=...` показывает в приложении безопасный
    итог серверной проверки заказа.
