...
import { ACCOUNT_DELETION_GRACE_MS, deletionRequestFingerprint, evaluateDeletionRisk, verifyDeletionAppCheck } from './account-deletion-workflow.js';
...
async function processAuthoredLineups(db, uid, subjectId) {
  ...
  let anonymized = 0;
  do {
    let query = db.collection('lineups').where('user_id', '==', uid).orderBy('__name__').limit(200);
...
export async function deleteAccountData({ db, auth, uid, subjectId, now }) {
  ...
  const directRoots = ['notifications', 'user_private', 'user_stats', 'user_auth_links'];
  for (const collection of directRoots) {
    await db.recursiveDelete(db.collection(collection).doc(uid));
  }
  const batch = db.batch();
  for (const collection of ['account_entitlements', 'user_public_perks', 'user_badges', 'rate_limits']) {
    ...
    try {
      ...
      if (pending.exists && pending.data()?.status === 'scheduled') {
        return res.status(200).json({
          scheduled: true,
          execute_after: pending.data().execute_after.toDate().toISOString()
        });
      }
      const serverNow = now();
      const userRecord = await authService.getUser(decoded.uid);
      const appCheckVerified = await verifyDeletionAppCheck(appCheck ?? adminApp().appCheck(), req.headers?.['x-firebase-appcheck']);
      const risk = evaluateDeletionRisk({
        decoded,
        userRecord,
        appCheckVerified,
        priorRequests: Number(pending.data()?.request_count || 0),
        now: serverNow
      });
      const executeAfter = new Date(serverNow.getTime() + ACCOUNT_DELETION_GRACE_MS);
      await requestRef.set({
        uid: decoded.uid,
        subject_id: deletedSubjectId(decoded.uid, pepper),
        status: 'scheduled',
        requested_at: Timestamp.fromDate(serverNow),
        execute_after: Timestamp.fromDate(executeAfter),
...
export function createAdminAccountDeleteHandler({
  db = null, auth = null, pepper = process.env.ACCOUNT_DELETION_PEPPER,
  now = () => new Date(), deleteData = deleteAccountData
} = {}) {
  return async function adminAccountDeleteHandler(req, res) {
    res.setHeader('Cache-Control', 'no-store');
    try {
      ...
      const { decoded } = await requireAdminRequest(req, {
        auth: authService,
        db: store
      });
      const uid = String(req.body?.uid || '').trim();
      if (!uid) throw fail(400, 'target_uid_required');
      if (uid === decoded.uid) throw fail(400, 'use_self_service_deletion');
      if (req.body?.confirm !== true) throw fail(400, 'confirmation_required');

      const subjectId = deletedSubjectId(uid, pepper);
      const lineups = await deleteData({
        db: store,
        auth: authService,
        uid,
        subjectId,
      ...
      await store.collection('deleted_accounts').doc(subjectId).set(
        {
      ...
      return res.status(200).json({ deleted: true, uid, lineups });
    } catch (error) {
      return adminRequestError(res, error, 'admin-account-delete');
    }
  };
}
