...
import { createGuildWebsite } from './guild-ui.mjs?v=2026-08-25-guild-v14';
import {
  canSubmitForRewards,
...
function renderRewardDialog() {
  ...
  const payoutAction = rewardDebt > 0
    ? `<div class="reward-empty">Запрос кода временно недоступен: нужно погасить ${rewardDebt} VP, отозванные за снятый с публикации лайнап. Следующие награды сначала погасят этот остаток.</div>`
    : !accepting
...
async function requestModeratorRewardDecision(lineupId) {
  ...
  return new Promise(resolve => {
    ...
    overlay.innerHTML = `<section class="reward-review-dialog" role="dialog" aria-modal="true" aria-labelledby="reward-review-title">
      ...
      <div class="reward-review-auto"><span>Общий дефицит на карте</span><b class="${rewardContext.deficit?.global ? 'yes' : 'no'}">${rewardContext.deficit?.global ? '✓ Да · +1 VP' : '✕ Нет'}</b><span>Дефицит активного маппула</span><b class="${rewardContext.deficit?.map_pool ? 'yes' : 'no'}">${rewardContext.deficit?.map_pool ? '✓ Да · +2 VP' : '✕ Нет'}</b><span>Автозадание рынка</span><b class="${rewardContext.matched_task ? 'yes' : 'no'}">${rewardContext.matched_task ? `✓ ${esc(String(rewardContext.matched_task.zone || '').toUpperCase() || 'Да')} · +1 VP` : '✕ Нет'}</b><span>Предварительный итог</span><b class="total" data-final-reward-total>—</b></div>
...
function rejectedLineupDraft(item) {
  ...
  return {
    ...
    spikeUsage: item.spike_usage || null,
    spikeX: item.spike_x ?? null,
    spikeY: item.spike_y ?? null,
    title: item.title || '',
